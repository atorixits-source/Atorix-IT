...
// Add this near the top with other schema definitions
...
app.get('/api/hiring-leads/count', async (req, res) => {
  try {
    const HiringLeadModel = mongoose.models.HiringLead || mongoose.model('HiringLead', HiringLeadSchema);
    const count = await HiringLeadModel.countDocuments();
    ...
    res.status(500).json({
      success: false,
      message: 'Error counting hiring leads',
      error: error.message
    });
  }
});
...
// --- HIRING LEADS ENDPOINTS ---


// GET hiring leads (using job applications)
app.get('/api/hiring-leads', async (req, res) => {
  console.log('Fetching hiring leads (job applications)...');
  try {
    // Check database connection
    if (mongoose.connection.readyState !== 1) {
      console.error('Database not connected!');
    ...
    // Map job application fields to match the expected hiring leads format
    const hiringLeads = applications.map(app => ({
      _id: app._id,
      name: app.fullName,
      email: app.email,
      phone: app.phone,
      ...
      // Include additional fields from job application
    ...
    return res.json({ 
      success: true, 
      count: hiringLeads.length,
      data: hiringLeads 
    });
    
  } catch (error) {
    console.error('Error in /api/hiring-leads:', error);
    return res.status(500).json({
      success: false,
...
app.post('/api/hiring-leads', upload.single('resume'), async (req, res) => {
  console.log('=== Hiring Lead Received ===');
  console.log('Request body:', JSON.stringify(req.body, null, 2));
  ...
  try {
    ...
    console.log('Hiring lead saved successfully:', hiringLead._id);
    ...
    res.status(201).json({
      success: true,
      message: 'Hiring lead submitted successfully',
      data: hiringLead
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to save hiring lead',
      error: error.message
    });
  }
});
...
// GET hiring leads count (using job applications)
app.get('/api/hiring-leads/count', async (req, res) => {
  try {
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   - POST   /api/demo-requests - Submit demo request`);
  console.log(`   - GET    /api/hiring-leads - Get hiring leads`);
  console.log('========================================');
